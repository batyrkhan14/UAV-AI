...
public class AeroplaneController : NetworkBehaviour
{
	...
	// The aeroplane's height above the ground.
	public float Throttle { get; private set; }
	// The amount of throttle being used.
	public bool AirBrakes { get; private set; }
	// Whether or not the air brakes are being applied.
	public float ForwardSpeed { get; private set; }
	// How fast the aeroplane is traveling in it's forward direction.
	public float EnginePower { get; private set; }
	// How much power the engine is being given.
	...
	[Command (channel = 1)]
	void CmdSyncParams (float Altitude_, float Throttle_, bool AirBrakes_, float ForwardSpeed_, 
	                    float EnginePower_, float m_MaxEnginePower_, float RollAngle_, float PitchAngle_, 
	                    float RollInput_, float PitchInput_, float YawInput_, float ThrottleInput_)
	{
		RpcSyncParams (Altitude_, Throttle_, AirBrakes_, ForwardSpeed_, EnginePower_, 
			m_MaxEnginePower_, RollAngle_, PitchAngle_, RollInput_, PitchInput_, YawInput_, ThrottleInput_);
	}

	[ClientRpc (channel = 1)]
	void RpcSyncParams (float Altitude_, float Throttle_, bool AirBrakes_, float ForwardSpeed_, 
	                    float EnginePower_, float m_MaxEnginePower_, float RollAngle_, float PitchAngle_, 
	                    float RollInput_, float PitchInput_, float YawInput_, float ThrottleInput_)
	{
		if (!isLocalPlayer) {
			...
			ForwardSpeed = ForwardSpeed_;
			EnginePower = EnginePower_;
			m_MaxEnginePower = m_MaxEnginePower_;
			RollAngle = RollAngle_;
			PitchAngle = PitchAngle_;
			RollInput = RollInput_;
			PitchInput = PitchInput_;
			YawInput = YawInput_;
			ThrottleInput = ThrottleInput_;
		}
	}


	private float m_OriginalDrag;
	// The drag when the scene starts.
	private float m_OriginalAngularDrag;
	// The angular drag when the scene starts.
	private float m_AeroFactor;
	private bool m_Immobilized = false;
	// used for making the plane uncontrollable, i.e. if it has been hit or crashed.
	...
	public void Move (float rollInput, float pitchInput, float yawInput, float throttleInput, bool airBrakes)
	{
		...
		CmdSyncParams (Altitude, Throttle, AirBrakes, ForwardSpeed, EnginePower, 
			m_MaxEnginePower, RollAngle, PitchAngle, RollInput, PitchInput, YawInput, ThrottleInput);
	}
